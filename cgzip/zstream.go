...
int cgzipGetHeader(char *strm, char *h) {
	return inflateGetHeader((z_stream*)strm, (gz_header *)h);
}

void cgzipSetHeaderExtra(char *h, void *buf, unsigned int len) {
  ((gz_header*)h)->extra = (Bytef*)buf;
  ((gz_header*)h)->extra_max = len;
}

Bytef *cgzipGetHeaderPointer(char *h) {
	return (Bytef *) (((gz_header *)h)->extra);
}

unsigned int cgzipGetHeaderLen(char *h) {
	return ((gz_header*)h)->extra_len;
}
...
type zheader [unsafe.Sizeof(C.gz_header{})]C.char

func (zhdr *zheader) declareHeaderExtra(buffer []byte) {
	C.cgzipSetHeaderExtra(&zhdr[0], unsafe.Pointer(&buffer[0]), C.uint(len(buffer)))
}

func (zhdr *zheader) getHeaderExtra(strm *zstream) error {
	result := C.cgzipGetHeader(&strm[0], &zhdr[0])
	if result != Z_OK {
		return fmt.Errorf("cgzip: failed to request extra header (%v): %v", result, strm.msg())
	}
	return nil
}
